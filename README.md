...
## Security?
AES encryption can be enabled and key exchange is facilitated by ECDH. You are on your own for authentication and message integrity. The server endpoint is designed to be ephemeral; you set it up and you yourself connect the client to it. If you need additional security, there are many proxies available that can perform TLS termination/offloading (NGINX/STUNNEL for example).
